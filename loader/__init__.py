...
def get_dataset(data_dict, split_type=None, data_aug=None, dequant=None):
    ...
    if name == 'MNISTLeaveOut':
        ...
        dataset = KMNIST(root=data_path, train=split_type == 'training', download=True, transform=ToTensor())
    elif name == 'EMNIST':
        ...
        tr = Compose([
            ...
            ToTensor()])
        dataset = EMNIST(root=data_path, split='letters', train=split_type == 'training', download=True,
                transform=tr)
